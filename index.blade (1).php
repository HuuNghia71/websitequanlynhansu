...
const currentNhanVienId = @json(optional(auth()->user())->id ?? 1);
...
function loadChamCongData() {
    ...
    const maNhanVien = document.getElementById("filterMaNhanVien").value.trim();
            ...
            if (!data || data.length === 0) {
                ...
                data.forEach(item => {
                    ...
                    html += `
                        <tr>
                            <td>${item.Ngay ?? '--'}</td>
                            <td>${item.MaNhanVien ?? item.ma_nhan_vien ?? '--'}</td>
                            <td class="text-success">${item.GioVao ?? '--:--'}</td>
                            <td class="text-danger">${item.GioRa ?? '--:--'}</td>
                            <td>${item.SoPhutTre ?? 0}</td>
                            <td>${item.SoGioLam ?? 0}</td>
                            <td>${item.SoNgayCong ?? 0}</td>
                            <td>${item.SoGioTangCa ?? 0}</td>
                            <td>
                                ${
                                    item.LaNgayLe == 1
...
function handleCheckIn() {
    ...
    if (!confirm("Bạn có chắc muốn Check-in?")) {
        return;
    }
...
function handleCheckOut() {
    ...
    .then(response => response.json())
    .then(data => {
        alert(data.message || "Check-out thành công");
        loadChamCongData();
    })
    .catch(error => {
        console.log(error);
